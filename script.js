...
function updSmoothProg(){
  const s=window.scrollY,d=document.documentElement.scrollHeight-window.innerHeight;
  targetScrollY = d>0?s/d:0;
}
window.addEventListener('scroll',updSmoothProg,{passive:true});

function smoothParallaxAnim() {
  currentScrollY += (targetScrollY - currentScrollY) * 0.08;
  document.body.style.setProperty('--scroll-y', currentScrollY.toFixed(4));
  requestAnimationFrame(smoothParallaxAnim);
}
smoothParallaxAnim();
